#!/usr/bin/env python3
"""
Download Images from Pexels API for Orson Vision
Downloads 1 image per section using the Pexels API (using requests library)
"""
...
def get_pexels_image_url(search_term):
    """Get image URL from Pexels API using requests"""
    try:
...
def main():
    print("="*60)
    print("🖼️  Downloading Images from Pexels API (requests)")
    print("="*60)
    success_count = 0
    ...
    for folder, search_term in SECTION_IMAGES.items():
        ...
        image_url = get_pexels_image_url(search_term)
        if image_url:
            filename = f"pexels_{search_term.replace(' ', '_')}.jpg"
