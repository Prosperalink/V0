#!/usr/bin/env python3
"""
Download Images from Pixabay API for Orson Vision
Downloads 1 image per section using the Pixabay API (using requests library)
"""
...
# Pixabay API Key
PIXABAY_API_KEY = "iJOcZebzVaISr7kLFc6VwLinxpIC7Lsk8v6dZ4okdIetld1mT3Tm83Ea"
...
def get_pixabay_image_url(search_term):
    """Get image URL from Pixabay API using requests"""
    try:
        search_url = f"https://pixabay.com/api/?key={PIXABAY_API_KEY}&q={search_term}&image_type=photo&orientation=horizontal&per_page=1"
...
def main():
    print("="*60)
    print("🖼️  Downloading Images from Pixabay API (requests)")
    print("="*60)
    ...
    for folder, search_term in SECTION_IMAGES.items():
        ...
        image_url = get_pixabay_image_url(search_term)
        if image_url:
            filename = f"pixabay_{search_term.replace(' ', '_')}.jpg"
